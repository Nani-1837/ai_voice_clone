...
export function HeroCarousel({
  ...
  return (
    <div
      ...
      aria-label="Featured movie dubs"
      onKeyDown={(e) => {
      ...
      className={cn(
        "relative h-full min-h-[30rem] sm:min-h-[36rem] w-full overflow-hidden bg-white text-slate-900 select-none border-y border-slate-200 shadow-lg",
        "outline-none focus-visible:ring-1 focus-visible:ring-purple-500/40 focus-visible:ring-inset",
        className
      )}
    >
      ...
      {/* ── Top bar: centred cluster ── */}
      <div
        className="absolute inset-x-0 flex items-center justify-center font-bold text-slate-900"
      ...
      >
        {onBack ? (
          <button
            type="button"
            onClick={onBack}
            className="opacity-90 transition-opacity hover:opacity-100 font-extrabold text-black hover:text-purple-700"
        ...
        {brand ? (
          <div
            className="font-black tracking-[0.08em] text-slate-900 bg-white/80 px-4 py-1.5 rounded-full border border-slate-200 shadow-xs"
      ...
      >
        <div className="flex w-full flex-wrap items-end gap-x-[6vw] gap-y-2">
          <AnimatePresence mode="popLayout" initial={false}>
            <motion.h2
              key={index}
              className="font-black leading-[0.92] tracking-[-0.03em] text-slate-900 drop-shadow-xs"
          ...
          {active.credit ? (
            <motion.p
              key={`credit-${index}`}
              className="font-mono uppercase tracking-[0.14em] text-purple-900 font-extrabold bg-purple-100/90 px-3 py-1 rounded-lg border border-purple-200"
          ...
          {active.meta?.length ? (
            ...
            >
              {active.meta.map((fact, i) => (
                <motion.span
                  key={`${index}-${fact}`}
                  className="font-mono whitespace-nowrap uppercase tracking-[0.14em] text-slate-900 font-extrabold bg-white/90 px-2.5 py-1 rounded border border-slate-200 shadow-xs"
                  style={{ fontSize: label }}
          ...
          {items.map((item, i) => (
            ...
            >
              ...
              <motion.span
                ...
                animate={{ opacity: i === index ? 0 : 0.15 }}
                transition={spring}
              />
            </motion.button>
          ))}
      ...
      >
        <div
          className="flex justify-between font-mono tabular-nums font-extrabold text-slate-900"
          style={{ fontSize: label }}
        >
          <span>{String(index + 1).padStart(2, "0")}</span>
          <span>{String(items.length).padStart(2, "0")}</span>
        </div>
        <div className="relative mt-2 h-1 w-full bg-slate-200 rounded-full overflow-hidden">
          <motion.div
            className="absolute inset-y-0 bg-gradient-to-r from-purple-600 to-cyan-500 rounded-full"
